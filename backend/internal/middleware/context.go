...
func NewContextEnchancer(s *server.Server) *ContextEnhancer {
	return &ContextEnhancer{server: s}
}

func (ce *ContextEnhancer) EnchanceContext() echo.MiddlewareFunc {
	return func(next echo.HandlerFunc) echo.HandlerFunc {
		return func(c echo.Context) error {
			// Extract request ID
			requestID := GetRequestID(c)

			// Create enchanced logger with request context
...
func (ce *ContextEnhancer) extractUserRole(c echo.Context) string {
	// Check if user_role was sent by auth middleware (Clerk)
